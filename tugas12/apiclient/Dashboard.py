...
class Dashboard:
    ...
    def menuAdmin(self):
        ...
        admin_menu.add_command(
            label='Data Petugas', command= lambda: self.new_window("Aplikasi Data Petugas", FrmAdmin)
        )
        admin_menu.add_command(
            label='Data Anggota', command= lambda: self.new_window("Aplikasi Data Anggota", FrmAnggota)
        )
        admin_menu.add_command(
            label='Data Buku', command= lambda: self.new_window("AplikasiData Buku", FrmBuku)
        )
        admin_menu.add_command(
            label='Peminjaman', command= lambda: self.new_window("Aplikasi Data Peminjaman", FrmPeminjaman)
        )
...
if __name__ == '__main__':
    root = tk.Tk()
    my_str = tk.StringVar()
    aplikasi = Dashboard(root, "Aplikasi Perpustkaan")
    root.mainloop() 